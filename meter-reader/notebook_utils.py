...
def load_image(path: str) -> np.ndarray:
    ...
    if path.startswith("http"):
        ...
        # Ignore bandit blacklist on the next call to urlopen
        # The protocol is being validated to start with http
        response = urllib.request.urlopen(request)  # nosec blacklist
...
def download_file(
    ...
    try:
        ...
        # Ignore bandit blacklist on the next call to urlopen
        # The url is being validated to start with http
        urlobject = urllib.request.urlopen(url, timeout=timeout)  # nosec blacklist
        if filename is None:
            filename = (
    ...
    if not filename.exists() or (os.stat(filename).st_size != urlobject_size):
        ...
        # Ignore bandit blacklist on the next call to urlretrieve
        # The url is being validated to start with http
        urllib.request.urlretrieve(  # nosec blacklist
...
def segmentation_map_to_overlay(
    ...
    """
    Returns a new image where a segmentation mask (created with colormap) is overlaid on
    the source image.
    ...
    :param colormap: A numpy array of shape (num_classes, 3) with an RGB value per class.
    :param remove_holes: If True, remove holes in the segmentation result.
    :return: An RGP image with segmentation mask overlaid on the source image.
    """
